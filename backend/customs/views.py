...
class RecommendationView(viewsets.GenericViewSet,
    ...
    def top_recommendation(self, request, *args, **kargs):
        region = request.query_params.get('region', 'Москва')
        instance = Recommendation.objects.prefetch_related('region').filter(region__region_name=region).values(
...
class TextAnalytic(viewsets.GenericViewSet,
    ...
    def export_to_presentation(self, request, *args, **kwargs):
        ...
        try:
            ...
            three = {'country_name': "Санкций нет"}
        main_partners = [three['country_name']]
